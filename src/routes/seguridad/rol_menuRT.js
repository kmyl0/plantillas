...
module.exports = app => {
  ...
  const sequelize = app.src.db.sequelize;
    ...
    @apiParam (Parametro) {Numerico} idRol Identificador del rol sobre el cual se quiere obtener los menus.

    @apiSuccess (Respuesta) {Numerico} id_rol_menu Identificador del rol-menu.
    @apiSuccess (Respuesta) {Numerico} fid_rol Identificador del rol.
    @apiSuccess (Respuesta) {Numerico} fid_menu Identificador del menu.
    ...
    @apiParam (Peticion) {Objeto} rol Objeto con informacion sobre el rol a crear.
    @apiParam (Peticion) {Vector} menus Vector con informacion de los menus asignados para el rol a crear.
    @apiParam (Peticion) {Texto} nombre Nombre del rol.
    @apiParam (Peticion) {Texto} descripcion descripcion del rol a crear.
    @apiParam (Peticion) {Numerico} peso Peso del rol.
    @apiParam (Peticion) {Numerico} _usuario_creacion Identificador del usuario creador.
    ...
    */
    ...
    @apiParam (Peticion) {Objeto} rol Objeto con informacion sobre el rol que se quiere actualizar.
    @apiParam (Peticion) {Objeto} menus Objeto con informacion sobre los menus a mantener e inactivar.
    @apiParam (Peticion) {Vector} menus_nuevos Vector con informacion de los nuevos menus asignados para el rol a crear.
    @apiParam (Peticion) {Numerico} id_rol Identificador del rol.
    @apiParam (Peticion) {Texto} nombre Nombre del rol.
    @apiParam (Peticion) {Texto} descripcion descripcion del rol.
    @apiParam (Peticion) {Numerico} peso Peso del rol.
    @apiParam (Peticion) {Texto} estado informacion del estado del rol.
    @apiParam (Peticion) {Numerico} _usuario_modificacion Identificador del usuario que modifica.

    @apiParam (Peticion) {Numerico} fid_rol_menu Identificador del rol-menu.
    @apiParam (Peticion) {Numerico} fid_rol Identificador del rol.
    @apiParam (Peticion) {Numerico} fid_menu Identificador del menu.
    @apiParam (Peticion) {Texto} estado informacion del estado del rol.
    @apiParam (Peticion) {Numerico} _usuario_creacion Identificador del usuario creador.
    @apiParam (Peticion) {Numerico} _usuario_modificacion Identificador del usuario que modifica.
